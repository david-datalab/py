...
#For example, given two lists of numbers, you might like to add them up pairwise,
#taking [3, 4, 5] and [1, 2, 3] to yield [4, 6, 8].
...
#  In Python, however, we have gotten used to the for loop where the iteration variable 
# is bound successively to each item in the list,
#  rather than just to a number that’s used as a position or index into the list.
#Can’t we do something similar with pairs of lists? It turns out we can.
#The zip function takes multiple lists and turns them into
#  a list of tuples (actually, an iterator, but they work like lists for most practical purposes), 
...
#Consider a function called possible, which determines whether 
# a word is still possible to play in a game of hangman,
...
L3 = [x1 + x2 for (x1, x2) in list(zip(L1, L2)) if x1 > 10 if x2 < 5]
print(L3)
